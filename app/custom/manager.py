...
class BaseSecurityManager(AbstractSecurityManager):
    ...
    @property
    def auth_ldap_role_field(self):
        return self.appbuilder.get_app.config["AUTH_LDAP_ROLE_FIELD"]
            ...
            try:
                ...
                # User does not exist, create one if self registration.
                    ...
                    if self.auth_user_registration and user is None:
                        ldap_role = self.ldap_extract(
                            ldap_user_info,
                            self.auth_ldap_role_field,
                            username
                        )

                        if ldap_role == "wattDoce":
                            watt_role = "doce_watt"
                        elif ldap_role == "wattTamm":
                            watt_role = "tamm_watt"
